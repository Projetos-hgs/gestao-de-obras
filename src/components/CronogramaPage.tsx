...
export function CronogramaPage() {
  ...
  async function updateProgress(id: string, value: number) {
    ...
    try { await api.schedule.update(id, { progress: value }); } catch { /* silent */ }
  }
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-white/5 bg-[#181B22]">
                      {['Empresa', 'Escopo', 'Contrato', 'Valor', 'Pagamento', 'Prazo'].map(h => (
                        <th key={h}
                          className="px-4 py-3 text-left text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {contracts.map((c, i) => {
                      ...
                      return (
                        <tr key={c.id}
                          className={`border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors ${i % 2 === 0 ? 'bg-[#16191F]' : 'bg-[#181B22]'}`}>
