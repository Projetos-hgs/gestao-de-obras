...
import {
  Building2, ChevronRight, Plus, Pencil, Trash2, CheckCircle2,
  AlertTriangle, FileText, TrendingUp, Briefcase,
} from 'lucide-react';
...
function diffDays(start?: string | null) {
  if (!start) return 0;
...
export function CronogramaPage() {
  ...
  /* nome do projeto particionado para estilo visual */
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={`flex-shrink-0 mt-1 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-bold border ${
            project?.status === 'Em Andamento' || project?.status === 'ativo'
        ...
        {projects.length > 1 && (
          <div className="flex items-center gap-2 mt-1">
            <select
              ...
              className="appearance-none bg-transparent border-b border-white/10 text-xs text-zinc-400 pr-4 focus:outline-none cursor-pointer"
            >
              {projects.map(p => (
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        {topItems.slice(0, 3).map((item, idx) => {
          ...
          return (
            <div key={item.id} className="bg-[#1C1F26] border border-white/5 rounded-xl p-4 flex flex-col gap-1" style={{ borderTopColor: col }}>
              <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">{item.name.toUpperCase()}</p>
              <p className="text-2xl font-black" style={{ color: col }}>{item.progress}%</p>
              <p className="text-[10px] text-zinc-500">{labels[idx + 1] ?? ''}</p>
            </div>
          );
        })}
        ...
        <div className="bg-[#1C1F26] border border-white/5 rounded-xl p-4 flex flex-col gap-1">
          ...
          <p className="text-[10px] text-zinc-500">
            {pendingDocs === 0 ? 'Documentação em dia' : 'Aguardando aprovação'}
          </p>
        </div>
      </div>
          ...
          {items.length === 0 ? (
            ...
            items.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className="group flex items-center gap-4 px-4 py-3 bg-[#181B22] border border-white/5 rounded-lg hover:border-white/10 transition-colors"
                >
                  {/* Nome */}
                  <span className="w-52 flex-shrink-0 text-sm font-semibold text-zinc-200 truncate">
                    {item.name}
                  </span>
                  ...
                  <div className="flex items-center gap-2 w-20 justify-end">
                    <span className="text-xs font-bold" style={{ color: pct > 0 ? color : '#52525b' }}>
                      {pct}%
                    </span>
                    <input
                      type="range"
          ...
          {contracts.length === 0 ? (
            ...
            <div className="rounded-xl overflow-hidden border border-white/5">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {contracts.map((c, i) => {
                    const rawSigned = c.signed;
                    const signed = rawSigned === true || rawSigned === 'true' || rawSigned === 'sim'
                      ? true : rawSigned === false || rawSigned === 'false' || rawSigned === 'nao' || rawSigned === 'não'
                      ? false : null;
                    ...
                    const pagamento = c.installments ? `${c.installments} parcelas` : c.payment_terms ?? null;
                      ...
                      <tr key={c.id} className={`border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors ${i % 2 === 0 ? 'bg-[#16191F]' : 'bg-[#181B22]'}`}>
                        ...
                        <td className="px-4 py-3.5">
                          {signed === true ? (
                            ...
                            <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-zinc-800 text-zinc-500">–</span>
                          )}
                        </td>
                        <td className="px-4 py-3.5 font-bold whitespace-nowrap">
                          {hasValue ? <span className="text-emerald-400">{fmtBudget(c.value)}</span> : <span className="text-zinc-600">—</span>}
                        </td>
                        <td className="px-4 py-3.5 text-zinc-400 whitespace-nowrap">{pagamento ?? <span className="text-zinc-600">—</span>}</td>
